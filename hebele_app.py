        ...
        if str(df.at[i,'Destination Postal Code']) != str(df.at[i,'Origin Postal Code']) :
            if str(df.at[j,'Destination Postal Code']) != str(df.at[j,'Origin Postal Code']) :
                if ( df.at[i,'Delivery Date'] <= df.at[j,'Goods issue date'] ) :
                    if ( (df.at[j,'Goods issue date'] - df.at[i,'Delivery Date']).total_seconds() <= TOLERANCE*24*60*60 )  :
...
def checkmate(t,final):
    ...
    while devam == True :
        if ifClosed(0,final) == True:
            ...
            t = t + 1
            print(MAX,'km az mesafeye dönüldü')
            devam = True
            ...
            t = t + 1
            print(MAX,'km az mesafeye dönüldü 3')
            devam = True
...
def calculate2():
    ...
    for i, r1 in enumerate(routes):
        for j, r2 in enumerate(routes):
            if r1.origin == r2.destination and r2.origin == r1.destination and r1 != r2 and r1.origin != r1.destination and r2.origin != r2.destination:
                ...
                if time_difference < (TOLERANCE*24*60*60) and time_difference >= 0:
    ...
    for i, p1 in enumerate(pairs):
        ...
        for j, p2 in enumerate(pairs):
            ...
            if time_delta >= 0 and time_delta < (TOLERANCE*24*60*60) and not match.contains(p2):
...
window.title("DHL FTL")

#window.geometry('350x200')

lbl0 = Label(window, text="-")
lbl0.grid(column=0, row=0, columnspan=6, sticky=(N, S, E, W))

lbl1 = Label(window, text="TOLERANCE")
lbl1.grid(column=1, row=2, columnspan=6, sticky=(N, S, E, W))
entry1 = Entry(window, text="TOLERANCE")
entry1.grid(column=0, row=3, columnspan=6, sticky=(N, S, E, W))

lbl2 = Label(window, text="MAX DIST")
lbl2.grid(column=1, row=4, columnspan=6, sticky=(N, S, E, W))
entry2 = Entry(window, text="MAX DIST")
entry2.grid(column=0, row=5, columnspan=6, sticky=(N, S, E, W))

lbl3 = Label(window, text="MODEL  (A*(total_d - dist)/2 - B*(total_w) - C*(dist))")
lbl3.grid(column=1, row=6, columnspan=6, sticky=(N, S, E, W))

lbl1 = Label(window, text="A = ")
lbl1.grid(column=0, row=7)
lbl4 = Label(window, text="(total_d - dist)/2")
lbl4.grid(column=1, row=8)
entry3 = Entry(window, text="(total_d - dist)/2")
entry3.grid(column=1, row=7)

lbl1 = Label(window, text="B = ")
lbl1.grid(column=2, row=7)
lbl5 = Label(window, text="total_w")
lbl5.grid(column=3, row=8)
entry4 = Entry(window, text="total_w")
entry4.grid(column=3, row=7)

lbl1 = Label(window, text="C = ")
lbl1.grid(column=4, row=7)
lbl6 = Label(window, text="dist")
lbl6.grid(column=5, row=8)
entry5 = Entry(window, text="dist")
entry5.grid(column=5, row=7)
...
def getfile():
    global file
    file = tk.filedialog.askopenfilename(title = "Select file", filetypes = [("Excel files", "*.xlsx")])
    print(file)
...
btn2 = Button(window, text="RUN", command=clicked)
